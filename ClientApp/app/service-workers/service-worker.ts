(async function()
{
    ...
    workerSelf.addEventListener("fetch", function(event)
    {
        ...
        event.respondWith(
            ...
            {
                console.log(
                    `Couldn't get ${event.request}, network is down and it does not exist in the cache. [${error}]`);
            })
        );
    });
    ...
    workerSelf.addEventListener("message", async function(event)
    {
        ...
        const myClients = await workerSelf.clients.matchAll();
        console.log(myClients);
        const windowClient = myClients.find(i => true);
